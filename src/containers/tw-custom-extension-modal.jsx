...
class CustomExtensionModal extends React.Component {
    ...
    async handleLoadExtension () {
        ...
        try {
            ...
            if (this.isUnsandboxed()) {
                // Remember this choice (including a manually-approved untrusted URL) so the
                // extension actually loads unsandboxed and isn't re-prompted next time.
    ...
    canChangeUnsandboxed () {
        // Trusted URLs are always unsandboxed automatically; no need to let the user
        // toggle it. Everything else (including untrusted URLs) can be manually opted in.
        if (this.state.type === 'url') {
