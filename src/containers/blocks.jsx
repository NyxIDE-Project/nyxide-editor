...
import RemoveExtensionModal from '../components/tw-remove-extension-modal/remove-extension-modal.jsx';
import {getRemovableExtensionIds, countExtensionBlocks, removeExtension} from '../lib/tw-extension-blocks.js';
...
const messages = defineMessages({
    ...
    REMOVE_EXTENSION: {
        ...
        description: 'Right-click menu item to remove an extension and the blocks that use it',
        id: 'tw.removeExtension.menuItem'
    }
});
...
class Blocks extends React.Component {
    constructor (props) {
        ...
        bindAll(this, [
            ...
            'handleToolboxContextMenu',
            'handleRemoveExtensionMenuClick',
            'handleConfirmRemoveExtension',
            'handleCancelRemoveExtension'
        ]);
    ...
    componentDidMount () {
        ...
        // tw: Let users right-click an extension's category in the toolbox to remove it.
        this.workspace.toolbox_.HtmlDiv.addEventListener('contextmenu', this.handleToolboxContextMenu);
    ...
    shouldComponentUpdate (nextProps, nextState) {
        return (
            ...
            this.state.removeExtensionModal !== nextState.removeExtensionModal ||
    ...
    componentWillUnmount () {
        this.detachVM();
        this.unmounted = true;
        if (this.workspace.toolbox_ && this.workspace.toolbox_.HtmlDiv) {
            this.workspace.toolbox_.HtmlDiv.removeEventListener('contextmenu', this.handleToolboxContextMenu);
        }
    ...
    handleToolboxContextMenu (e) {
        const item = e.target.closest('.scratchCategoryMenuItem');
        if (!item) return;

        const idClass = Array.prototype.find.call(item.classList, c => c.startsWith('scratchCategoryId-'));
        if (!idClass) return;
        const extensionId = idClass.slice('scratchCategoryId-'.length);
        if (!getRemovableExtensionIds(this.props.vm).includes(extensionId)) return;

        const categoryInfo = this.props.vm.runtime._blockInfo.find(info => info.id === extensionId);
        const name = (categoryInfo && categoryInfo.name) || extensionId;
    ...
    handleRemoveExtensionMenuClick (e, data) {
        const blockCount = countExtensionBlocks(this.props.vm, data.extensionId);
        if (blockCount > 0) {
            this.setState({
            ...
            removeExtension(this.props.vm, data.extensionId);
        }
    }
    handleConfirmRemoveExtension () {
        removeExtension(this.props.vm, this.state.removeExtensionModal.extensionId);
        this.setState({removeExtensionModal: null});
    }
    handleCancelRemoveExtension () {
        this.setState({removeExtensionModal: null});
    }
    render () {
        ...
        return (
            <React.Fragment>
                ...
                <ContextMenu id="tw-extension-context-menu">
                    <DangerousMenuItem onClick={this.handleRemoveExtensionMenuClick}>
                        {this.props.intl.formatMessage(messages.REMOVE_EXTENSION)}
                    </DangerousMenuItem>
                </ContextMenu>
                {this.state.removeExtensionModal ? (
                    <RemoveExtensionModal
                        name={this.state.removeExtensionModal.name}
                        blockCount={this.state.removeExtensionModal.blockCount}
                        onCancel={this.handleCancelRemoveExtension}
                        onConfirm={this.handleConfirmRemoveExtension}
                    />
                ) : null}
            </React.Fragment>
        );
